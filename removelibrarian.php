...
if(isset($_POST['remove'])){
		$email=$_POST['email'];
		$mysqli= "SELECT * from register_librarian";
		$resul=mysqli_query($connect,$mysqli);
		$mysql="DELETE FROM register_librarian where email='".$email."'";
		$result=mysqli_query($connect,$mysql);
		$array=mysqli_fetch_array($resul);
		if ($email==$array['email']) {
			echo "you have sucessfully remove Librarian ".$array['fullname']."";
	
		}
		else{
			echo "failed to remove a Librarian";
		}
	}
}
?>
	</p>
	<h1 style="margin-left: 10%">Remove Librarian</h1><br>
<form method="POST">
