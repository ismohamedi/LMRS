...
if(isset($_POST['remove'])){
		$email=$_POST['email'];
		$mysqli= "SELECT * from register_staff";
		$resul=mysqli_query($connect,$mysqli);
		$mysql="DELETE FROM register_staff where email='".$email."'";
		$result=mysqli_query($connect,$mysql);
		$array=mysqli_fetch_array($resul);
		if ($email==$array['email']) {
			echo "you have sucessfully remove staff-member  ".$array['fullname']."";
	
		}
		else{
			echo "failed to remove a staff-member";
		}
	}
}
?>
	</p>
	<h1 style="margin-left: 10%">Remove staff-member</h1><br>
<form method="POST">
